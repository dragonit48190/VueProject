<template>
  <MainLayout>
      ...
      <div class="ps-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0 p-0">
            ...
            <li class="breadcrumb-item active" aria-current="page"> PHYSICAL PATIENT</li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12">
        <h6 class="mb-0 text-uppercase">ค้นหารายชื่อผู้รับบริการแผนกกายภาพบำบัด นอกเวลาราชการ</h6>
        <hr />
          ...
          <div class="card-body p-4">
            ...
            <div v-if="patients.length > 0" class="mt-3">
              <div class="alert alert-primary py-2">
                ...
                พบรายชื่อกายภาพบำบัด นอกเวลาราชการ <strong>{{ patients.length.toLocaleString() }}</strong> ราย 
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h6 class="mb-0 text-uppercase text-primary">
          ...
          รายชื่อแผนกกายภาพบำบัด นอกเวลาราชการ โรงพยาบาลโพนสวรรค์
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'อายุ', value: 'age' },  
        { text: 'น้ำหนัก', value: 'bw' },
        { text: 'สูง', value: 'height' },
        { text: 'วันที่', value: 'vstdate' },
        { text: 'เวลา', value: 'stime' },
        { text: 'รายการ', value: 'psname' },
        { text: 'ICD', value: 'icd' },
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const res = await api.get(API_ROUTES.PHYSICOVERTIME, {
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานผู้รับบริการกายภาพบำบัด นอกเวลาราชการ โรงพยาบาลโพนสวรรค์'],  
          ...
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'CID', 'ชื่อ-สกุล', 'อายุ', 'นน.', 'สูง', 'วันที่', 'เวลา', 'รายการ', 'ICD', 'ICDNAME', 'ผู้ให้บริการ','เลขที่', 'หมู่', 'ตำบล'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.cid, p.ptname, p.age, p.bw, p.height,  
            p.vstdate, p.stime, p.psname, p.icd, p.icdname, p.doctor, p.num, p.moo, p.tum
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Physical')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `PHYSICOVERTIME_${dayjs(this.dateRaw1).format('YYYYMMDD')}_to_${dayjs(this.dateRaw2).format('YYYYMMDD')}.xlsx`
