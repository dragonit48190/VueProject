<template>
  <MainLayout>
      ...
      <div class="ps-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0 p-0">
            ...
            <li class="breadcrumb-item active" aria-current="page">DENGUEFEVER ICD A90 - A919</li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12">
        <h6 class="mb-0 text-uppercase">ค้นหารายชื่อแพทย์วินิจฉัย ไข้เลือดออก DENGUEFEVER ICD A90 - A919 </h6>
        <hr />
          ...
          <div class="card-body p-4">
            ...
            <div v-if="patients.length > 0" class="mt-3">
              <div class="alert alert-success py-2">
                ...
                พบรายชื่อแพทย์วินิจฉัยไข้เลือดออก DENGUE ICD A90 - A919 <strong>{{ patients.length.toLocaleString() }}</strong> ราย 
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h6 class="mb-0 text-uppercase text-primary">
          ...
          รายชื่อแพทย์วินิจฉัย ไข้เลือดออก DENGUE ICD A90 - A919 โรงพยาบาลโพนสวรรค์
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'ICD', value: 'icd' },
        { text: 'โรค', value: 'icdname' },
        { text: 'HCT', value: 'HCT' },
        { text: 'WBC', value: 'WBC' },
        { text: 'PLTCount', value: 'PLTCount' },
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const res = await api.get(API_ROUTES.DENGUEFEVER, {
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานแพทย์วินิจฉัย ไข้เลือดออก DENGUE ICD A90 - A919 โรงพยาบาลโพนสวรรค์'],  // แก้ข้อความ
          [`ช่วงวันที่: ${dateRange}`],  // เพิ่มช่วงวันที่
          [`วันที่ออกรายงาน: ${dayjs().format('DD/MM/YYYY HH:mm:ss')}`],
          [`จำนวนทั้งหมด: ${this.patients.length} ราย`],
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'CID', 'ชื่อ-สกุล', 'อายุ', 'เพศ','วันที่', 'ICD', 'ICDNAME', 'HCT', 'WBC', 'PLTCount', 'เลขที่', 'หมู่', 'ตำบล', 'รพ.สต.'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.cid, p.ptname, p.age, p.sex, p.vstdate, 
            p.icd, p.icdname, p.hct, p.wbc, p.PLTCount, p.num, p.moo, p.tum, p.hospital
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'DENGUE ICD A90 - A919')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `DENGUE_${dayjs(this.dateRaw1).format('YYYYMMDD')}_to_${dayjs(this.dateRaw2).format('YYYYMMDD')}.xlsx`
