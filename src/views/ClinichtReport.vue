        ...
        <div class="ps-3">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
              ...
              <li class="breadcrumb-item active" aria-current="page">CLINIC HT</li>
            </ol>
          </nav>
        </div>
      </div>
            ...
            <div class="card-body p-4">
              <div class="card-title d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center">
                  ...
                  <h5 class="mb-0 text-primary">รายชื่อผู้ป่วยความดันโลหิตสูง ลงทะเบียน Clinic HT</h5>
                </div>
                <button 
              ...
              <div class="row">
                <div class="col-md-6">
                  ...
                  <p class="mb-1"><strong>คลินิก :</strong> ความดันโลหิตสูง (HT Clinic)</p>
                  <p class="mb-0"><strong>สถานะ :</strong> สมาชิกที่ยังไม่ได้ลงวันจำหน่าย</p>
                  <p class="mb-0"><strong>จำนวน :</strong> {{ patients.length.toLocaleString() }} ราย </p>

                </div>
                <div class="col-md-6">
                  <!-- สถิติจำนวนผู้ป่วย -->
                  <div class="alert bg-primary text-white text-white py-2 mb-0 mt-2">
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'เพศ', value: 'sex' },
        { text: 'อายุ', value: 'age' },
        { text: 'วันลงทะเบียน', value: 'regdate' },
        { text: 'Note', value: 'note' },
        { text: 'ปีที่เป็น', value: 'begin_year' },
  ...
  methods: {
    async fetchData() {
      try {
        this.loading = true
        const res = await api.get(API_ROUTES.CLINIC_HT)
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานลงทะเบียนผู้ป่วยความดันโลหิตสูง HT CLINIC โรงพยาบาลโพนสวรรค์'],
          [`วันที่ออกรายงาน: ${dayjs().format('DD/MM/YYYY HH:mm:ss')}`],
          [`จำนวนผู้ป่วยทั้งหมด: ${this.patients.length} ราย`],
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'CID', 'ชื่อ-สกุล', 'เพศ', 'อายุ', 'วันที่ลงทะเบียน', 'Note', 'ปีที่เป็น', 'Clinic', 'สถานะ', 'เลขที่', 'หมู่', 'ตำบล', 'รพ.สต.'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.cid, p.ptname, p.sex, p.age, p.regdate, 
            p.note, p.begin_year, p.clinic, p.status, p.num, p.moo, p.tum, p.hospital
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Clinic HT')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `ClinicHT_${dayjs().format('YYYYMMDD_HHmmss')}.xlsx`
