<template>
  <MainLayout>
      ...
      <div class="ps-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0 p-0">
            ...
            <li class="breadcrumb-item active" aria-current="page">RAPID COVID-19</li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12">
        <h6 class="mb-0 text-uppercase">ค้นหารายชื่อตรวจ LAB RAPID COVID-19 </h6>
        <hr />
          ...
          <div class="card-body p-4">
            ...
            <div v-if="patients.length > 0" class="mt-3">
              <div class="alert alert-success py-2">
                ...
                พบรายชื่อตรวจ Rapid Covid-19 <strong>{{ patients.length.toLocaleString() }}</strong> ราย 
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h6 class="mb-0 text-uppercase text-primary">
          ...
          รายชื่อตรวจ Lab Rapid Covid-19 โรงพยาบาลโพนสวรรค์
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'Lab', value: 'rapid' },
        { text: 'ผล Lab', value: 'result' },  
        { text: 'เลขที่', value: 'num' },
        { text: 'หมู่', value: 'moo' },
        { text: 'ตำบล', value: 'tum' },
        { text: 'อำเภอ', value: 'amp' },
        { text: 'จังหวัด', value: 'chw' },
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const res = await api.get(API_ROUTES.RAPID, {
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานตรวจ LAB RAPID COVID-19 โรงพยาบาลโพนสวรรค์'],  // แก้ข้อความ
          ...
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'CID', 'ชื่อ-สกุล', 'อายุ', 'วันที่', 'เวลา', 'ICD', 'ประวัติ', 'LAB', 'ผล LAB','เลขที่', 'หมู่', 'ตำบล', 'อำเภอ' , 'จังหวัด', 'รพ.สต.'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.cid, p.ptname, p.age, p.vstdate, p.vsttime, 
            p.icd, p.cc, p.rapid, p.result, p.num, p.moo, p.tum, p.amp, p.chw, p.hospital
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'RAPID COVID-19')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `RAPID_${dayjs(this.dateRaw1).format('YYYYMMDD')}_to_${dayjs(this.dateRaw2).format('YYYYMMDD')}.xlsx`
