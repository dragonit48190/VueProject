<template>
  <MainLayout>
      ...
      <div class="ps-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0 p-0">
            ...
            <li class="breadcrumb-item active" aria-current="page">DENGUE ICD A90 - A919 Phonsawan</li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12">
        <h6 class="mb-0 text-uppercase">ค้นหารายชื่อแพทย์วินิจฉัย ไข้เลือดออก DENGUE ICD A90 - A919 ต.โพนสวรรค์</h6>
        <hr />
          ...
          <div class="card-body p-4">
            ...
            <div v-if="patients.length > 0" class="mt-3">
              <div class="alert alert-success py-2">
                ...
                พบรายชื่อแพทย์วินิจฉัยไข้เลือดออก DENGUE ICD A90 - A919 ต.โพนสวรรค์ <strong>{{ patients.length.toLocaleString() }}</strong> ราย 
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h6 class="mb-0 text-uppercase text-primary">
          ...
          รายชื่อแพทย์วินิจฉัย ไข้เลือดออก DENGUE ICD A90 - A919 ต.โพนสวรรค์
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const res = await api.get(API_ROUTES.DENGUE, {
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานแพทย์วินิจฉัย ไข้เลือดออก DENGUE ICD A90 - A919 ต.โพนสวรรค์'],  
          ...
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'CID', 'ชื่อ-สกุล', 'อายุ', 'เพศ','วันที่', 'ICD', 'ICDNAME', 'HCT', 'WBC', 'PLTCount', 'เลขที่', 'หมู่', 'บ้าน'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.cid, p.ptname, p.age, p.sex, p.vstdate, 
            p.icd, p.icdname, p.hct, p.wbc, p.PLTCount, p.num, p.moo, p.ban
          ])
        ]
