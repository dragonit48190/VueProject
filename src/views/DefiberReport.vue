<template>
  <MainLayout>
      ...
      <div class="ps-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0 p-0">
            ...
            <li class="breadcrumb-item active" aria-current="page"> Defibrillation ER </li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12">
        <h6 class="mb-0 text-uppercase">ค้นหารายชื่อทำหัตถการ Defibrillation ER</h6>
        <hr />
          ...
          <div class="card-body p-4">
            ...
            <div v-if="patients.length > 0" class="mt-3">
              <div class="alert alert-success py-2">
                ...
                พบรายชื่อทำหัตถการ Defibrillation ER<strong>{{ patients.length.toLocaleString() }}</strong> ราย 
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h6 class="mb-0 text-uppercase text-primary">
          ...
          รายชื่อทำหัตถการ Defibrillation ER  โรงพยาบาลโพนสวรรค์
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'ICD', value: 'icd' },
        { text: 'ER-TYPE', value: 'ertype' },
        { text: 'หัตถการ', value: 'defib' },
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const res = await api.get(API_ROUTES.DEFIBER, {
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานทำหัตการ Defibrillation ER  โรงพยาบาลโพนสวรรค์'],  
          ...
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'CID', 'ชื่อ-สกุล', 'อายุ', 'วันที่', 'ICD', 'ER-TYPE', 'หัตถการ', 'สถานะ'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.cid, p.ptname, p.age, p.vstdate, 
            p.icd, p.ertype, p.defib, p.status
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Defibrillation ER')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `DEFIBER_${dayjs(this.dateRaw1).format('YYYYMMDD')}_to_${dayjs(this.dateRaw2).format('YYYYMMDD')}.xlsx`
