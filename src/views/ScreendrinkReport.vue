        ...
        <div class="ps-3">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
              ...
              <li class="breadcrumb-item active" aria-current="page">SCREEN DRINKING</li>
            </ol>
          </nav>
        </div>
      </div>
            ...
            <div class="card-body p-4">
              <div class="card-title d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center">
                  ...
                  <h5 class="mb-0 text-primary">รายชื่อคัดกรองสูบบุหรี่ ดื่มสุรา ต.โพนสวรรค์ </h5>
                </div>
                <button 
              ...
              <div class="row">
                <div class="col-md-6">
                  ...
                  <p class="mb-1"><strong>คัดกรอง :</strong> สูบบุหรี่ ดื่มสุรา ต.โพนสวรรค์</p>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'สูง', value: 'hi' },
        { text: 'นน.', value: 'bw' },
        { text: 'รอบเอว', value: 'waist' },
        { text: 'สูบบุหรี่', value: 'smoke' },
        { text: 'ดื่มสุรา', value: 'drink' },
  ...
  methods: {
    async fetchData() {
      try {
        this.loading = true
        const res = await api.get(API_ROUTES.SCREENDRINK)
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานคัดกรองสูบบุหรี่ ดื่มสุรา ตำบลโพนสวรรค์'],
          [`วันที่ออกรายงาน: ${dayjs().format('DD/MM/YYYY HH:mm:ss')}`],
          [`จำนวนผู้ป่วยทั้งหมด: ${this.patients.length} ราย`],
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'CID', 'ชื่อ-สกุล', 'เพศ', 'อายุ', 'สูง', 'นน.', 'รอบเอว', 'สูบบุหรี่', 'ดื่มสุรา', 'วันที่','เลขที่', 'หมู่', 'บ้าน'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.cid, p.ptname, p.sex, p.age, p.hi, 
            p.bw, p.waist, p.smoke, p.drink, p.vstdate, p.num, p.moo, p.ban
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Screen Smoking & Drinking')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `SCREENDRINK_${dayjs().format('YYYYMMDD_HHmmss')}.xlsx`
