        ...
        <div class="ps-3">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
              ...
              <li class="breadcrumb-item active" aria-current="page">CLINIC SHIZO</li>
            </ol>
          </nav>
        </div>
      </div>
            ...
            <div class="card-body p-4">
              <div class="card-title d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center">
                  ...
                  <h5 class="mb-0 text-primary">รายชื่อลงทะเบียน คลินิกสุขภาพจิต </h5>
                </div>
                <button 
              ...
              <div class="row">
                <div class="col-md-6">
                  ...
                  <p class="mb-1"><strong>คลินิก :</strong> สุขภาพจิต (Clinic Shizo)</p>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'ปีที่เป็น', value: 'byear' },
        { text: 'วันที่จำหน่าย', value: 'dchdate' },
        { text: 'วันที่นัด', value: 'nextdate' },
  ...
  methods: {
    async fetchData() {
      try {
        this.loading = true
        const res = await api.get(API_ROUTES.CLINICSHIZO)
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานลงทะเบียนคลินิกสุขภาพจิต โรงพยาบาลโพนสวรรค์'],
          [`วันที่ออกรายงาน: ${dayjs().format('DD/MM/YYYY HH:mm:ss')}`],
          [`จำนวนผู้ป่วยทั้งหมด: ${this.patients.length} ราย`],
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'CID', 'ชื่อ-สกุล', 'เพศ', 'อายุ', 'วันที่ลงทะเบียน', 'Note', 'ปีที่เป็น', 'วันที่จำหน่าย', 'วันที่นัด', 'Clinic', 'เลขที่', 'หมู่', 'ตำบล', 'รพ.สต.'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.cid, p.ptname, p.sex, p.age, p.regdate, 
            p.note, p.byear, p.dchdate, p.nextdate, p.clinic, p.num, p.moo, p.tum, p.hospital
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Clinic Shizo')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `ClinicShizo_${dayjs().format('YYYYMMDD_HHmmss')}.xlsx`
