<template>
  <MainLayout>
      ...
      <div class="ps-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0 p-0">
            ...
            <li class="breadcrumb-item active" aria-current="page">NEW COPD</li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12">
        <h6 class="mb-0 text-uppercase">ค้นหารายชื่อผู้ป่วยปอดอุดกั้นเรื้อรังรายใหม่ (NEW COPD)</h6>
        <hr />
          ...
          <div class="card-body p-4">
            ...
            <div v-if="patients.length > 0" class="mt-3">
              <div class="alert alert-success py-2">
                ...
                พบผู้ป่วยปอดอุดกั้นเรื้อรังรายใหม่ <strong>{{ patients.length.toLocaleString() }}</strong> ราย 
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h6 class="mb-0 text-uppercase text-primary">
          ...
          รายชื่อผู้ป่วยปอดอุดกั้นเรื้อรังรายใหม่ (NEW COPD) - โรงพยาบาลโพนสวรรค์
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'อายุ', value: 'age' },  // แก้จาก Age
        { text: 'วันที่วินิจฉัย', value: 'vstdate' },  // เพิ่มความชัดเจน
        { text: 'ICD', value: 'icd' },
        { text: 'ชื่อโรค', value: 'icdname' },  // แก้จาก ICDNAME
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const res = await api.get(API_ROUTES.NEW_COPD, {
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานผู้ป่วยปอดอุดกั้นเรื้อรังรายใหม่ (NEW COPD) โรงพยาบาลโพนสวรรค์'],  // แก้ข้อความ
          ...
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'CID', 'ชื่อ-สกุล', 'อายุ', 'วันที่วินิจฉัย', 'ICD', 'ชื่อโรค', 'เลขที่', 'หมู่', 'ตำบล', 'รพ.สต.'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.cid, p.ptname, p.age, p.vstdate, 
            p.icd, p.icdname, p.num, p.moo, p.tum, p.hospital
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'NEW COPD')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `NewCOPD_${dayjs(this.dateRaw1).format('YYYYMMDD')}_to_${dayjs(this.dateRaw2).format('YYYYMMDD')}.xlsx`
