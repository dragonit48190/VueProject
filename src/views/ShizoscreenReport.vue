<template>
  <MainLayout>
      ...
      <div class="ps-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0 p-0">
            ...
            <li class="breadcrumb-item active" aria-current="page"> SHIZOSCREEN Z503</li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12">
        <h6 class="mb-0 text-uppercase">ค้นหารายชื่อจิตเวชคัดกรอง Z503</h6>
        <hr />
          ...
          <div class="card-body p-4">
            ...
            <div v-if="patients.length > 0" class="mt-3">
              <div class="alert alert-success py-2">
                ...
                พบรายชื่อคัดกรอง Z503 <strong>{{ patients.length.toLocaleString() }}</strong> ครั้ง 
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h6 class="mb-0 text-uppercase text-success">
          ...
          รายชื่อผู้ป่วยจิตเวชคัดกรอง Z503 โรงพยาบาลโพนสวรรค์
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const res = await api.get(API_ROUTES.SHIZOSCREEN, {
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานจิตเวชคัดกรอง Z503 โรงพยาบาลโพนสวรรค์'],  
          ...
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'CID', 'ชื่อ-สกุล', 'เพศ', 'อายุ', 'วันที่', 'เวลา', 'Pdx', 'ICD', 'ICDNAME','บ้านเลขที่', 'หมู่', 'ตำบล', 'อำเภอ', 'จังหวัด'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.cid, p.ptname, p.sex, p.age,
            p.vstdate, p.time, p.pdx, p.icd, p.icdname, p.num, p.moo, p.tum , p.amp, p.chw
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Shizo Screen Z503')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `Z503_${dayjs(this.dateRaw1).format('YYYYMMDD')}_to_${dayjs(this.dateRaw2).format('YYYYMMDD')}.xlsx`
