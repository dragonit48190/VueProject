<template>
  <MainLayout>
      ...
      <div class="ps-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0 p-0">
            ...
            <li class="breadcrumb-item active" aria-current="page">DM SCREEN FOOT</li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12">
        <h6 class="mb-0 text-uppercase">ค้นหารายชื่อผู้ป่วยความดันโลหิตสูงคัดกรองเท้า (DM SCREEN FOOT)</h6>
        <hr />
          ...
          <div class="card-body p-4">
            ...
            <div v-if="patients.length > 0" class="mt-3">
              <div class="alert alert-success py-2">
                ...
                พบผู้ป่วยความดันโลหิตสูงคัดกรองเท้า <strong>{{ patients.length.toLocaleString() }}</strong> ราย 
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h6 class="mb-0 text-uppercase text-primary">
          ...
          รายชื่อผู้ป่วยความดันโลหิตสูงคัดกรองเท้า (DM SCREEN FOOT) - โรงพยาบาลโพนสวรรค์
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'วันที่วินิจฉัย', value: 'screen_date' },  
        { text: 'เท้าซ้าย', value: 'left_foot' },
        { text: 'เท้าขวา', value: 'right_foot' },
        { text: 'ผล', value: 'current_ulcer' }, 
        { text: 'ประวัติ', value: 'ประวัติ' },
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const res = await api.get(API_ROUTES.DMSCREENFOOT, {
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานผู้ป่วยความดันโลหิตสูงคัดกรองเท้า (DM SCREEN FOOT) โรงพยาบาลโพนสวรรค์'],  // แก้ข้อความ
          ...
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'CID', 'ชื่อ-สกุล', 'อายุ', 'screen', 'วันที่', 'เท้าซ้าย', 'เท้าขวา', 'ผล',
           'ประวัติ', 'เลขที่', 'หมู่', 'ตำบล', 'รพ.สต.'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.cid, p.ptname, p.age, p.screen, p.vstdate, 
            p.LF, p.RF, p.ulc, p.ulcn, p.num, p.moo, p.tum, p.hospital
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'DM SCREEN FOOT')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `NewHT_${dayjs(this.dateRaw1).format('YYYYMMDD')}_to_${dayjs(this.dateRaw2).format('YYYYMMDD')}.xlsx`
