<template>
  <MainLayout>
      ...
      <div class="ps-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0 p-0">
            ...
            <li class="breadcrumb-item active" aria-current="page"> ADMIT 2 HOUR</li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12">
        <h6 class="mb-0 text-uppercase">ค้นหารายชื่อ Admit 2 hour </h6>
        <hr />
          ...
          <div class="card-body p-4">
            ...
            <div v-if="patients.length > 0" class="mt-3">
              <div class="alert alert-primary py-2">
                ...
                พบรายชื่อ Admit 2 hour <strong>{{ patients.length.toLocaleString() }}</strong> ราย 
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h6 class="mb-0 text-uppercase text-primary">
          ...
          รายชื่อ Admit 2 hour โรงพยาบาลโพนสวรรค์
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'เวลา', value: 'regtime' },
        { text: 'วันที่กลับ', value: 'dchdate' },
        { text: 'เวลากลับ', value: 'dchtime' },
        { text: 'สถานะ', value: 'status' },
        { text: 'ICD', value: 'icd' },
        { text: 'ICDNAME', value: 'icdname' },
        { text: 'รวมเวลา', value: 'hour' },
      ],
      config: {
        locale: Thai,
        dateFormat: 'Y-m-d',
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const res = await api.get(API_ROUTES.ADMIT2HOUR, {
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงาน Admit 2 hour โรงพยาบาลโพนสวรรค์'],  
          ...
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'AN','HN', 'CID', 'ชื่อ-สกุล', 'อายุ', 'วันที่รับ', 'เวลารับ', 'วันที่กลับ', 'เวลากลับ', 'สถานะ', 'ICD', 'ICDNAME', 'จำนวน ชม.'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.an, p.hn, p.cid, p.ptname, p.age, p.regdate, p.regtime, p.dchdate, p.dchtime, 
            p.status, p.icd, p.icdname, p.hour
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Admit 2 Hour')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `ADMIT2HOUR_${dayjs(this.dateRaw1).format('YYYYMMDD')}_to_${dayjs(this.dateRaw2).format('YYYYMMDD')}.xlsx`
