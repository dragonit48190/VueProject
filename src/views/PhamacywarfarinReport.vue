        ...
        <div class="ps-3">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
              ...
              <li class="breadcrumb-item active" aria-current="page">WARFARIN DRUG</li>
            </ol>
          </nav>
        </div>
      </div>
            ...
            <div class="card-body p-4">
              <div class="card-title d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center">
                  ...
                  <h5 class="mb-0 text-primary">รายชื่อผู้ป่วยรับยา Warfarin รพ.โพนสวรรค์ </h5>
                </div>
                <button 
              ...
              <div class="row">
                <div class="col-md-6">
                  ...
                  <p class="mb-1"><strong>รับยา :</strong> Warfarin</p>
...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        this.loading = true
        const res = await api.get(API_ROUTES.PHAMACYWARFARIN)
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานลงทะเบียนแพ้ยา โรงพยาบาลโพนสวรรค์'],
          [`วันที่ออกรายงาน: ${dayjs().format('DD/MM/YYYY HH:mm:ss')}`],
          [`จำนวนผู้ป่วยทั้งหมด: ${this.patients.length} ราย`],
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'ชื่อ-สกุล', 'เพศ', 'อายุ', 'วันเกิด', 'ยา', 'วันที่', 'เลขที่', 'หมู่', 'ตำบล', 'รพ.สต.'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.ptname, p.sex, p.age, p.birthdate, p.drug, 
            p.vstdate, p.num, p.moo, p.tum, p.hospital
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Warfarin Drug')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `WarfarinDrug_${dayjs().format('YYYYMMDD_HHmmss')}.xlsx`
