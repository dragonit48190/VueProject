        ...
        <div class="ps-3">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
              ...
              <li class="breadcrumb-item active" aria-current="page">Alert Drug</li>
            </ol>
          </nav>
        </div>
      </div>
            ...
            <div class="card-body p-4">
              <div class="card-title d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center">
                  ...
                  <h5 class="mb-0 text-primary">รายชื่อผู้ป่วยลงทะเบียนแพ้ยา รพ.โพนสวรรค์ </h5>
                </div>
                <button 
              ...
              <div class="row">
                <div class="col-md-6">
                  ...
                  <p class="mb-1"><strong>ทะเบียน :</strong> แพ้ยา Alert Drug</p>
...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        this.loading = true
        const res = await api.get(API_ROUTES.PHAMACYALERT)
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ...
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'ชื่อ-สกุล', 'วันเกิด', 'แพ้ยา', 'อาการ', 'Note', 'วันที่', 'เลขที่', 'หมู่', 'ตำบล', 'รพ.สต.'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.ptname, p.birthdate, p.agent, 
            p.symptom, p.note, p.vstdate, p.num, p.moo, p.tum, p.hospital
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Alert Drug')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `AlertDrug_${dayjs().format('YYYYMMDD_HHmmss')}.xlsx`
