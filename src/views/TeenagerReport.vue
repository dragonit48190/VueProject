        ...
        <div class="ps-3">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
              ...
              <li class="breadcrumb-item active" aria-current="page">TEENAGER REPORT</li>
            </ol>
          </nav>
        </div>
      </div>
            ...
            <div class="card-body p-4">
              <div class="card-title d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center">
                  ...
                  <h5 class="mb-0 text-primary">รายชื่อกลุ่มวัยรุ่น อายุ 15 - 21 ปี</h5>
                </div>
                <button 
              ...
              <div class="row">
                <div class="col-md-6">
                  ...
                  <p class="mb-1"><strong>คลินิก :</strong> วัยรุ่นอายุ 15 - 21 ปี</p>
...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        this.loading = true
        const res = await api.get(API_ROUTES.TEENAGER)
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานกลุ่มอายุ 15 - 21 ปี  โรงพยาบาลโพนสวรรค์'],
          [`วันที่ออกรายงาน: ${dayjs().format('DD/MM/YYYY HH:mm:ss')}`],
          [`จำนวนทั้งหมด: ${this.patients.length} ราย`],
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'CID', 'ชื่อ-สกุล', 'เพศ', 'วันเกิด','อายุ', 'เดือน', 'วัน', 'สิทธิ', 'เลขที่', 'หมู่', 'บ้าน', 'สถานะ'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.cid, p.ptname, p.sex, p.birthdate, p.age, p.month, 
            p.day, p.tname, p.num, p.moo, p.ban, p.status
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'TEENAGER 15-21')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `TEENAGER_${dayjs().format('YYYYMMDD_HHmmss')}.xlsx`
