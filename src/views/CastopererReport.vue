<template>
  <MainLayout>
      ...
      <div class="ps-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0 p-0">
            ...
            <li class="breadcrumb-item active" aria-current="page"> SPLINT ER </li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12">
        <h6 class="mb-0 text-uppercase">ค้นหารายชื่อใส่เฝือก ER</h6>
        <hr />
          ...
          <div class="card-body p-4">
            ...
            <div v-if="patients.length > 0" class="mt-3">
              <div class="alert alert-success py-2">
                ...
                พบรายชื่อใส่เฝือก ER<strong>{{ patients.length.toLocaleString() }}</strong> ราย 
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h6 class="mb-0 text-uppercase text-primary">
          ...
          รายชื่อใส่เฝือก ER  โรงพยาบาลโพนสวรรค์
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const res = await api.get(API_ROUTES.CASTOPERER, {
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานใส่เฝือก ER  โรงพยาบาลโพนสวรรค์'],  
          ...
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'CID', 'ชื่อ-สกุล', 'อายุ', 'วันที่', 'ICD', 'ER-Type', 'เฝือก', 'สถานะ'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.cid, p.ptname, p.age, p.vstdate, 
            p.icd, p.ertype, p.splint, p.status
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'SPLINT ER')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `SPLINTER_${dayjs(this.dateRaw1).format('YYYYMMDD')}_to_${dayjs(this.dateRaw2).format('YYYYMMDD')}.xlsx`
