<template>
  <MainLayout>
      ...
      <div class="ps-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0 p-0">
            ...
            <li class="breadcrumb-item active" aria-current="page">DM SCREEN EYE</li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12">
        <h6 class="mb-0 text-uppercase">ค้นหารายชื่อผู้ป่วยเบาหวานคัดกรองตา (DM SCREEN EYE)</h6>
        <hr />
          ...
          <div class="card-body p-4">
            ...
            <div v-if="patients.length > 0" class="mt-3">
              <div class="alert alert-success py-2">
                ...
                พบผู้ป่วยเบาหวานคัดกรองตา <strong>{{ patients.length.toLocaleString() }}</strong> ราย 
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h6 class="mb-0 text-uppercase text-primary">
          ...
          รายชื่อผู้ป่วยเบาหวานคัดกรองตา (DM SCREEN EYE) - โรงพยาบาลโพนสวรรค์
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'idtype', value: 'idtype' },
        { text: 'type', value: 'type' },
        { text: 'type', value: 'LE' }, 
        { text: 'type', value: 'RE' }, 
        { text: 'Vale', value: 'vale' },   
        { text: 'varig', value: 'varig' },
        { text: 'tex', value: 'tex' },
        { text: 'หมู่', value: 'moo' },
        { text: 'ตำบล', value: 'tum' },
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const res = await api.get(API_ROUTES.DMSCREENEYE, {
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานผู้ป่วยเบาหวานคัดกรองตา (DM SCREEN EYE) โรงพยาบาลโพนสวรรค์'],  // แก้ข้อความ
          ...
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'CID', 'ชื่อ-สกุล', 'อายุ', 'screen', 'วันที่', 'idtype', 'type', 'LE', 'RE', 'vale', 'varig', 'tex', 'เลขที่', 'หมู่', 'ตำบล', 'รพ.สต.'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.cid, p.ptname, p.age, p.screen, p.vstdate, 
            p.idtype, p.type, p.LE, p.RE, p.vale, p.varig, p.tex, p.num, p.moo, p.tum, p.hospital
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'DM SCREEN EYE')
