<template>
  <MainLayout>
      ...
      <div class="ps-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0 p-0">
            ...
            <li class="breadcrumb-item active" aria-current="page">PDX NULL ER</li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12">
        <h6 class="mb-0 text-uppercase">ค้นหารายชื่อยังไม่มีวินิจฉัย ER (PDX NULL ER)</h6>
        <hr />
          ...
          <div class="card-body p-4">
            ...
            <div v-if="patients.length > 0" class="mt-3">
              <div class="alert alert-success py-2">
                ...
                พบรายชื่อยังไม่ลงวินิจฉัย  <strong>{{ patients.length.toLocaleString() }}</strong> ราย 
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h6 class="mb-0 text-uppercase text-primary">
          ...
          รายชื่อผู้ป่วยยังไม่ลงวินิจฉัย ER (PDX NULL ER) - โรงพยาบาลโพนสวรรค์
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const res = await api.get(API_ROUTES.PDXNULLER, {
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานยังไม่ลงวินิจฉัย ER  โรงพยาบาลโพนสวรรค์'],  
          ...
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'CID', 'ชื่อ-สกุล', 'เพศ','อายุ', 'วันที่', 'เวลา', 'สิทธิ', 'ICD', 'ซักประวัติ', 'ค่ารักษา', 'แผนก', 'ผู้ส่งตรวจ'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.cid, p.ptname, p.sex, p.age, p.vstdate, p. 
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'PDX NULL ER')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `PDXNULLER_${dayjs(this.dateRaw1).format('YYYYMMDD')}_to_${dayjs(this.dateRaw2).format('YYYYMMDD')}.xlsx`
