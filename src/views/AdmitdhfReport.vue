<template>
  <MainLayout>
      ...
      <div class="ps-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0 p-0">
            ...
            <li class="breadcrumb-item active" aria-current="page"> ADMIT Dengue</li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12">
        <h6 class="mb-0 text-uppercase">ค้นหารายชื่อ Admit Dengue</h6>
        <hr />
          ...
          <div class="card-body p-4">
            ...
            <div v-if="patients.length > 0" class="mt-3">
              <div class="alert alert-primary py-2">
                ...
                พบรายชื่อ Admit Dengue <strong>{{ patients.length.toLocaleString() }}</strong> ราย 
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h6 class="mb-0 text-uppercase text-primary">
          ...
          รายชื่อ Admit Dengue โรงพยาบาลโพนสวรรค์
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const res = await api.get(API_ROUTES.ADMITDHF, {
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงาน Admit Dengue โรงพยาบาลโพนสวรรค์'],  
          ...
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'AN','HN', 'CID', 'ชื่อ-สกุล', 'อายุ', 'วันที่รับ', 'วันที่กลับ', 'ICD', 'Dx1', 'Dx2', 'ICDNAME', 'เลขที่', 'หมู่', 'ตำบล', 'รพ.สต.'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.an, p.hn, p.cid, p.ptname, p.age, p.regdate, p.dchdate,  
            p.icd, p.dx0, p.dx1, p.icdname, p.num, p.moo, p.tum, p.hospital
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Admit Dengue')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `ADMITDHF_${dayjs(this.dateRaw1).format('YYYYMMDD')}_to_${dayjs(this.dateRaw2).format('YYYYMMDD')}.xlsx`
