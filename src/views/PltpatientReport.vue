<template>
  <MainLayout>
      ...
      <div class="ps-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0 p-0">
            ...
            <li class="breadcrumb-item active" aria-current="page"> PLANTTHAI PATIENT</li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-12">
        <h6 class="mb-0 text-uppercase">ค้นหารายชื่อผู้รับบริการแผนกแพทย์แผนไทย</h6>
        <hr />
          ...
          <div class="card-body p-4">
            ...
            <div v-if="patients.length > 0" class="mt-3">
              <div class="alert alert-success py-2">
                ...
                พบรายชื่อผู้ป่วยแพทย์แผนไทย <strong>{{ patients.length.toLocaleString() }}</strong> ครั้ง 
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h6 class="mb-0 text-uppercase text-success">
          ...
          รายชื่อผู้ป่วยแพทย์แผนไทย โรงพยาบาลโพนสวรรค์
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'ยา', value: 'drug' },
        { text: 'เวชภัณฑ์', value: 'nondrug' },
        { text: 'จำนวน', value: 'qty' },
        { text: 'ค่ารักษา', value: 'price' },
        { text: 'สิทธิรักษา', value: 'pttype' }
      ],
      config: {
        locale: Thai,
        dateFormat: 'Y-m-d',
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const res = await api.get(API_ROUTES.PLTPATIENT, {
    ...
    exportToExcel() {
      try {
        ...
        const excelData = [
          ['รายงานผู้รับบริการแพทย์แผนไทย โรงพยาบาลโพนสวรรค์'],  
          ...
          [], // บรรทัดว่าง
          // Headers
          ['ลำดับ', 'HN', 'CID', 'ชื่อ-สกุล', 'เพศ', 'อายุ', 'วันที่', 'รายการยา', 'เวชภัณฑ์', 'จำนวน', 'ค่ารักษา','สิทธิ'],
          // Data
          ...this.patients.map(p => [
            p.sequence, p.hn, p.cid, p.ptname, p.sex, p.age,
            p.vstdate, p.drug, p.nondrug, p.qty, p.price
          ])
        ]
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Planthai Patient')
        
        // กำหนดชื่อไฟล์และดาวน์โหลด
        const fileName = `PLTPATIENT_${dayjs(this.dateRaw1).format('YYYYMMDD')}_to_${dayjs(this.dateRaw2).format('YYYYMMDD')}.xlsx`
