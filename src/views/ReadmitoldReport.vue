<template>
  <MainLayout>
      ...
      <div class="ps-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0 p-0">
            ...
            <li class="breadcrumb-item active" aria-current="page">Re-ADMIT 28 วัน ด้วยโรคเดิม </li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="row row-cols-1 row-cols-2">
      <div class="col-12">
        <h6 class="mb-0 text-uppercase">ค้นหารายชื่อแพทย์ Re-ADMIT 28 วัน ด้วยโรคเดิม </h6>
        <hr />
    ...
    <h6 class="mb-0 text-uppercase text-primary">รายชื่อแพทย์ Re-ADMIT ด้วยโรคเดิม โรงพยาบาลโพนสวรรค์ </h6>
    <hr />
    <div class="card">
      <div class="card-body">
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'Age', value: 'age' },
        { text: 'First_AN', value: 'first_an' },
        { text: 'First_Pdx', value: 'first_pdx' },
        { text: 'First_Dx', value: 'first_diagnosis' },
        { text: 'First_admit', value: 'first_admit' },
        { text: 'First_discharge', value: 'first_discharge' },
        { text: 'Readmit_an', value: 'readmit_an' },
        { text: 'Readmit_pdx', value: 'readmit_pdx' },
        { text: 'Readmit_diagnosis', value: 'readmit_diagnosis' },
        { text: 'Readmit_date', value: 'readmit_date' },
        { text: 'Days', value: 'days' },
        { text: 'Discharge_status', value: 'discharge_status' }
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const res = await api.get(API_ROUTES.RE_ADMITOLD, {
  ...
  try {
    ...
    const title = [
      ['รายงานผู้ป่วย RE_ADMIT_OLD โรงพยาบาลโพนสวรรค์'],
    ...
    const headers = [
      ['ลำดับ', 'HN', 'ชื่อ-สกุล', 'อายุ', 'first_an', 'first_pdx', 'first_diagnosis','first_admit', 'first_discharge',
       'readmit_an', 'readmit_pdx','readmit_diagnosis', 'readmit_date', 'days', 'discharge_status']
    ];
    ...
    const data = this.patients.map(patient => [
      ...
      patient.first_an,
      patient.first_pdx,
      patient.first_diagnosis,
      patient.first_admit,
      patient.first_discharge,
      patient.readmit_an,
      patient.readmit_pdx,
      patient.readmit_diagnosis,
      patient.readmit_date,
      patient.days,
    ...
    const wscols = [
      ...
      { wch: 6 }, // First_pdx
      { wch: 20 }, // First_Dx
      { wch: 25 }, // first_diagnosis
      { wch: 25 }, // first_admit
      { wch: 25 },  // first_discharge
      { wch: 6 }, // readmit_an
      { wch: 20 }, // readmit_pdx
      { wch: 30 }, // readmit_diagnosis
      { wch: 20 }, // readmit_date
      { wch: 8 }, // days
      { wch: 20 }, // discharge_status
    ];
    ws['!cols'] = wscols;
    
    // สร้าง Workbook
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Readmit');
    
    // สร้างชื่อไฟล์
    const d1 = this.dateRaw1 ? dayjs(this.dateRaw1).format('YYYYMMDD') : '';
    const d2 = this.dateRaw2 ? dayjs(this.dateRaw2).format('YYYYMMDD') : '';
    const fileName = `RE_ADMITOLD${d1}_to_${d2}.xlsx`;
