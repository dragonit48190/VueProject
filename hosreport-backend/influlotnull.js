...
// API Endpoint สำหรับดึงข้อมูลยังไม่ลง LOT INFLUENZA VACCINE
router.get('/influlotnull', (req, res) => {
  ...
  SELECT 
    os.icode,
    d.name AS vaccine,
    COALESCE(ov.vaccine_lot_no, 'ไม่ระบุ Lot') AS lot_vaccine,
    COUNT(DISTINCT o.vn) AS visit
FROM ovst o
LEFT JOIN opitemrece os on o.vn = os.vn
LEFT JOIN drugitems d ON os.icode = d.icode
LEFT JOIN ovst_vaccine ov ON ov.vn = o.vn 
WHERE os.icode = 1530003
    AND o.vstdate BETWEEN ? AND ?
GROUP BY os.icode, d.name, ov.vaccine_lot_no
ORDER BY COUNT(DISTINCT o.vn) DESC, ov.vaccine_lot_no
  `;
