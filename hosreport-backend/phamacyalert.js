...
router.get('/phamacyalert', (req, res) => {
  const sql = `
  SELECT
  p.hn,
  ...
  ol.agent,
  ol.symptom AS symptom,
  ol.note AS 'note',
  CONCAT(DATE_FORMAT(ol.report_date, '%d-%m-'), YEAR(ol.report_date) + 543) AS vstdate,
  p.addrpart AS num,
  p.moopart AS moo,
  t.NAME AS tum,
  CASE
...
FROM
  patient p
  INNER JOIN opd_allergy ol ON p.hn = ol.hn
  LEFT JOIN sex s ON p.sex = s.
  CODE LEFT JOIN thaiaddress t ON p.chwpart = t.chwpart
